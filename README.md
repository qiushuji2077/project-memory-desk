...
Project Memory Desk 是一个本地优先的项目记忆工作台。它帮助用户把项目过程中的零散信息沉淀为两层结构：右侧的“记忆流”保留过程痕迹，中间的“最终记忆”呈现当前可采用版本。它适合咨询交付、课题研究、产品设计、客户项目、科研项目、法律与合规材料等需要长期推进、多次沟通、多版文件、多轮决策的工作场景。

![Project Memory Desk demo](assets/demo-screenshot.png)

## 为什么做这个项目

在长期推进咨询方案、课题研究、产品设计、客户交付和合规材料时，资料经常分散在聊天、会议、录音、文件夹、PPT、Word 和多轮修改稿中。项目真正困难的地方并非保存更多资料，而是随时知道当前采用什么、哪些内容已被确认、哪些判断已被替代、下一步应该做什么。因此需要一个围绕“最终记忆”和“记忆流”组织项目上下文的本地优先工具。

## 当前版本功能

- 项目列表
- 最终记忆（一键复制整理提示词）
- 记忆流（按三流筛选：当前·待确认 / 已确认·原话 / 判断）
- 记忆流规范 v1 词表：定 / 判断 / 原话 / 待确认 / 废弃
- 相关文件 chips
- 提示词一键复制
- 手动录入记忆（按 `类型【来源】：内容` 解析）
- 导出记忆快照（Markdown）
- 匿名 demo 数据
- 本地运行
...
录入端统一一套封闭词表，只记"不可再生"的内容，把噪音挡在记忆流之外：

- **定**：已拍板、贯穿全篇的口径
- **判断**：影响后续取舍的专业判断（必须带"因为……"）
- **原话**：不可改写的原始表述（加引号）
- **待确认**：尚未拍板、会卡住后续的问题
- **废弃**：被推翻的旧结论，不进入最终记忆

统一格式 `类型【来源】：内容`，最终记忆每次从完整记忆流全量重建。完整规范见 [记忆流规范 v1](docs/memory-flow-spec.md)。
...
- [记忆流规范 v1](docs/memory-flow-spec.md)
